 ...
 */
router.post("/endpoints", authenticateToken, requireStaff, async (req: Request, res: Response) => {
  try {
    ...
    const endpoint = await WebhookRepository.createWebhookEndpoint({
      ...
      url,
      description,
      events: Array.isArray(events) ? (events as WebhookEventType[]) : [events as WebhookEventType],
      secret,
      headers,
      metadata,
    });
 ...
 */
router.get("/endpoints", authenticateToken, requireStaff, async (req: Request, res: Response) => {
  try {
    const clientId = (req as any).clientId;
    const activeOnly = req.query.active === "true";

    const endpoints = await WebhookRepository.getClientWebhookEndpoints(clientId, activeOnly);
 ...
 */
router.get("/endpoints/:id", authenticateToken, requireStaff, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const endpoint = await WebhookRepository.getWebhookEndpointById(new ObjectId(id));
    ...
    const stats = await WebhookRepository.getEndpointDeliveryStats(endpoint._id);
 ...
 */
router.patch("/endpoints/:id", authenticateToken, requireStaff, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { url, description, events, isActive, isTestMode, headers, metadata } = req.body;
    ...
    const updated = await WebhookRepository.updateWebhookEndpoint(new ObjectId(id), updateInput);
 ...
 */
router.delete("/endpoints/:id", authenticateToken, requireStaff, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const endpoint = await WebhookRepository.getWebhookEndpointById(new ObjectId(id));
 ...
 */
router.post("/endpoints/:id/test", authenticateToken, requireStaff, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const endpoint = await WebhookRepository.getWebhookEndpointById(new ObjectId(id));
 ...
 * List webhook events
 */
router.get("/events", authenticateToken, requireStaff, async (req: Request, res: Response) => {
  try {
    const clientId = (req as any).clientId;
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const deliveries = await WebhookRepository.getEndpointDeliveries(endpoint._id, {
        status: status as any,
        limit: parseInt(limit as string),
        skip: parseInt(skip as string),
      });

      const counts = await WebhookRepository.countDeliveriesByStatus((req as any).clientId);
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const delivery = await WebhookRepository.getWebhookDeliveryById(new ObjectId(deliveryId));
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (delivery.deliveryStatus === "success") {
        return res.status(400).json({ error: "Cannot retry successful delivery" });
      }
 ...
 */
router.get("/endpoints/:id/stats", authenticateToken, requireStaff, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const endpoint = await WebhookRepository.getWebhookEndpointById(new ObjectId(id));
    ...
    const stats = await WebhookRepository.getEndpointDeliveryStats(endpoint._id);
    const recentDeliveries = await WebhookRepository.getRecentDeliveries(endpoint._id, 24);
