...
export default function AdminReports() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (applicationsData.success) {
        setApplications(applicationsData.data || applicationsData.applications || []);
      }
  ...
  const calculateStats = () => {
    const filteredApplications = applications.filter(app => filterByDateRange(app.createdAt));
    const filteredPayments = payments.filter(payment => filterByDateRange(payment.date));
    const filteredUsers = users.filter(user => filterByDateRange(user.createdAt));
    ...
    // Application stats
    const approvedApplications = filteredApplications.filter(a => a.status === "approved").length;
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <Card>
                ...
                <CardContent className="space-y-4">
                  {stats && Object.entries(stats.applicationsByStatus).map(([status, count]) => (
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-semibold">{stats?.completedPayments || 0}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Clock className="w-4 h-4 text-yellow-600" />
              ...
              <Card>
                <CardHeader className="flex flex-row items-center justify-between">
                  <div>
                    ...
                    <CardDescription>Total revenue breakdown by service type</CardDescription>
                  </div>
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => {
                      if (stats?.revenueByService) {
                        const data = Object.entries(stats.revenueByService).map(([service, info]) => ({
                          Service: service,
                ...
                <CardContent className="space-y-4">
                  {stats && Object.entries(stats.revenueByService).length > 0 ? (
                      ...
                      .map(([service, info]) => (
                        <div key={service} className="space-y-2">
                          <div className="flex items-center justify-between">
                            ...
                            <span className="font-semibold">₹{info.revenue.toLocaleString()}</span>
                          </div>
                          <div className="flex items-center justify-between text-sm text-muted-foreground">
                            <span>{info.count} applications</span>
                            <span>₹{Math.round(info.revenue / info.count).toLocaleString()} avg</span>
                          </div>
                  ...
                  <Button
                    ...
                    onClick={() => {
                      if (stats?.revenueByMethod) {
                        const data = Object.entries(stats.revenueByMethod).map(([method, info]) => ({
                          Method: method,
                      ...
                      .map(([method, info]) => (
                        <div key={method} className="space-y-2">
                          <div className="flex items-center justify-between">
                            ...
                            <span className="font-semibold">₹{info.amount.toLocaleString()}</span>
                          </div>
                          <div className="flex items-center justify-between text-sm text-muted-foreground">
                            <span>{info.count} transactions</span>
                            <span>₹{Math.round(info.amount / info.count).toLocaleString()} avg</span>
                          </div>
                          <div className="w-full bg-secondary h-2 rounded-full overflow-hidden">
                            <div
                              className="bg-primary h-full"
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                <div>
                  ...
                  <CardDescription>Detailed application statistics and trends</CardDescription>
                </div>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => {
                    const filteredApps = applications.filter(app => filterByDateRange(app.createdAt));
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                <div>
                  ...
                  <CardDescription>Comprehensive payment analytics</CardDescription>
                </div>
                <Button
                  variant="outline"
                  size="sm"
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between p-3 bg-green-50 dark:bg-green-950 rounded-lg">
                        ...
                        <div className="text-right">
                          <p className="font-bold">{stats?.completedPayments || 0}</p>
                          <p className="text-sm text-muted-foreground">
                            ₹{stats?.totalRevenue.toLocaleString() || 0}
                          </p>
                        </div>
                      ...
                      <div className="flex items-center justify-between p-3 bg-yellow-50 dark:bg-yellow-950 rounded-lg">
                        ...
                        <div className="text-right">
                          <p className="font-bold">{stats?.pendingPayments || 0}</p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between p-3 bg-red-50 dark:bg-red-950 rounded-lg">
                        ...
                        <div className="text-right">
                          <p className="font-bold">{stats?.failedPayments || 0}</p>
                        </div>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="font-bold">
                          {stats && (stats.completedPayments + stats.pendingPayments + stats.failedPayments) > 0
                            ? Math.round(
                                (stats.completedPayments /
                                  (stats.completedPayments + stats.pendingPayments + stats.failedPayments)) *
                                  100
                              )
                            : 0}%
                        </span>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                <div>
                  ...
                  <CardDescription>User registration and distribution analytics</CardDescription>
                </div>
                <Button
                  variant="outline"
                  size="sm"
              ...
              <CardContent className="space-y-6">
                <div>
                  ...
                  <div className="space-y-4">
                    {stats && Object.entries(stats.usersByBusinessType).length > 0 ? (
                      Object.entries(stats.usersByBusinessType)
                        ...
                        .map(([type, count]) => (
                          <div key={type} className="space-y-2">
                            ...
                            <p className="text-sm text-muted-foreground">
                              {stats.totalUsers > 0 ? Math.round((count / stats.totalUsers) * 100) : 0}% of total users
                            </p>
                          </div>
                        ))
                    ) : (
